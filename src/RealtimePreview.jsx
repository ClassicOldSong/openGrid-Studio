...
export default function RealtimePreview({ mesh, loading, error, theme }) {
  ...
  function uploadMesh(meshData) {
    ...
    for (let i = 0; i < indices.length; i += 3) {
      ...
      const normal = vecNormalize(vecCross(
        [bx - ax, by - ay, bz - az],
        [cx - ax, cy - ay, cz - az],
      ))
    ...
    sceneRadius = Math.max(1, Math.hypot(
  ...
  function draw() {
    ...
    const translation = new Float32Array([
      sceneCenter[0] + focusX,
      sceneCenter[1] + focusY,
      sceneCenter[2] + focusZ,
    ])
  ...
  const statusClass = $(() => {
    ...
    const tone = rendererError.value || errorSig.value
  ...
  const resetButtonClass = $(() => loadingSig.value
  ...
  const backgroundStyle = $(() => themeSig.value === 'dark'
    ...
    <div class="relative h-full w-full overflow-hidden bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)] dark:bg-slate-950 dark:shadow-[0_24px_80px_rgba(2,6,23,0.6)]">
      ...
      <div class="absolute inset-x-0 top-0 z-10 flex items-start justify-between px-4 py-4">
        <div class="pointer-events-none">
          <div class="text-[10px] font-bold uppercase tracking-[0.24em] text-blue-600/80 dark:text-blue-300/80">Viewport</div>
        ...
        <canvas
          ...
          on:pointermove={(event) => {
            ...
            if (dragMode === 'pan') {
              ...
              focusX = dragStartFocus[0] + dragStartRight[0] * (-totalDx * dragStartScaleX) + dragStartUp[0] * (totalDy * dragStartScaleY)
              focusY = dragStartFocus[1] + dragStartRight[1] * (-totalDx * dragStartScaleX) + dragStartUp[1] * (totalDy * dragStartScaleY)
              focusZ = dragStartFocus[2] + dragStartRight[2] * (-totalDx * dragStartScaleX) + dragStartUp[2] * (totalDy * dragStartScaleY)
            } else {
